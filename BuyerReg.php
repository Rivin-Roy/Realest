    ...
    <div class="contact-infhny py-5">
        <div class="container py-lg-5">
            ...
            if (isset($_POST['submit'])) {
                ...
                $email = $_POST['email'];
                $psw = $_POST['psw'];
                $sel = mysql_query("SELECT COUNT(*) AS COUNT FROM `buyer` WHERE `phone`='$phone' OR `email`='$email'");
                $fetch = mysql_fetch_array($sel);
                if ($fetch['COUNT'] > 0) {
                    echo '<script>alert("Already Registered")</script>';
                } else {
                    $qry = "INSERT INTO `buyer`(`fname`,`lname`,`address`,`houno`,`city`,`dist`,`pin`,`phone`,`email`)values('$fname','$lname','$address','$houno','$city','$dist','$pin','$phone','$email')";

                    $exe = mysql_query($qry);
                    if ($qry) {
                        $qry2 = mysql_query("INSERT INTO `login`(`uid`,`uname`,`psw`,`type`)VALUES((SELECT MAX(`uid`)FROM `buyer`),'$email','$psw','buyer')");
